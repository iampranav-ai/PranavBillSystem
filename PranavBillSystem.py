...
root.title("Pranav Billing System")
root.configure(background='red')

Tops = Frame(root,bg='blue',bd=20,pady=5,relief=RIDGE)
Tops.pack(side=TOP)

TitleX=Label(Tops,font=('arial',60,'bold'),text='Pranav Billing System',bd=21,bg='black',
                fg='cornsilk',justify=CENTER)
TitleX.grid(row=0)


BillCal = Frame(root,bg='orange',bd=10,relief=RIDGE)
BillCal.pack(side=RIGHT)

Buttons_F=Frame(BillCal,bg='blue',bd=3,relief=RIDGE)
Buttons_F.pack(side=BOTTOM)

Cal_F=Frame(BillCal,bg='blue',bd=6,relief=RIDGE)
Cal_F.pack(side=TOP)

Receipt_F=Frame(BillCal,bg='red',bd=4,relief=RIDGE)
...
def CostofItem():
    ...
    PriceofDrinks =(Item1 * 20) + (Item2 * 25) + (Item3 * 30) + (Item4 * 35) + (Item5 * 40) + (Item6 * 45) + (Item7 * 50) + (Item8 * 55)

    PriceofFood =(Item9 * 30) + (Item10 * 40) + (Item11 * 50) + (Item12 * 60) + (Item13 * 40) + (Item14 * 30) + (Item15 * 50) + (Item16 * 45)
